...
设备规格书生成工具【云端稳定版】
修复说明：
1. 升级适配Python 3.14，修复底层渲染兼容bug
2. 全页面固定DOM结构，所有组件常驻渲染，零动态增删，彻底解决removeChild报错
3. 未启用/无数据时组件禁用灰化，不删除节点
4. 附录自动向前补位、双环境pandoc适配全部保留
"""
import os
import tempfile
...
# 固定3个自定义附录（D/E/F），DOM结构固定
if "dyn_appendix" not in st.session_state:
    st.session_state.dyn_appendix = [
        {"letter": "D", "name": "", "content": ""},
        {"letter": "E", "name": "", "content": ""},
        {"letter": "F", "name": "", "content": ""}
    ]
...
4. 附录规则：
   ...
   - 附录B：勾选启用接口清单表格；
   - 附录C：独立勾选开关控制是否启用；
   - 固定3个自定义附录D/E/F，可按需填写；
   - 未启用附录导出时自动跳过，后续附录序号向前补位，无空缺。
""")
    st.divider()
...
# 2 1 定义与缩略语
    ...
    selected = st.multiselect(
        "选择需要插入的术语/缩略语",
        options=opts,
        default=[opts[i] for i in st.session_state.db_selected_idx["dict"]] if opts else [],
        key="sel_dict_main",
        disabled=len(opts) == 0
    )
    st.session_state.db_selected_idx["dict"] = [opts.index(s) for s in selected] if opts else []
    if not opts:
...
# 4 3 供货与服务范围
    ...
    selected_mat = st.multiselect(
        "", options=opts_mat,
        default=[opts_mat[i] for i in st.session_state.db_selected_idx["material"]] if opts_mat else [],
        key="sel_material",
        disabled=len(opts_mat) == 0
    )
    st.session_state.db_selected_idx["material"] = [opts_mat.index(s) for s in selected_mat] if opts_mat else []
    if not opts_mat:
        st.info("未检测到【设备物料码】数据库文件")
...
# 5 4 适用文件
    ...
    with col1:
        ...
        sel_law = st.multiselect(
            "选择条目", opts_law,
            default=[opts_law[i] for i in st.session_state.db_selected_idx["law_l12"]] if opts_law else [],
            key="sel_law_l12",
            disabled=len(opts_law) == 0
        )
        st.session_state.db_selected_idx["law_l12"] = [opts_law.index(s) for s in sel_law] if opts_law else []
        if not opts_law:
            st.info("暂无数据")
    ...
    with col2:
        ...
        sel_std = st.multiselect(
            "选择条目", opts_std,
            default=[opts_std[i] for i in st.session_state.db_selected_idx["law_l345"]] if opts_std else [],
            key="sel_law_l345",
            disabled=len(opts_std) == 0
        )
        st.session_state.db_selected_idx["law_l345"] = [opts_std.index(s) for s in sel_std] if opts_std else []
        if not opts_std:
            st.info("暂无数据")
    ...
    with col3:
        ...
        sel_ref = st.multiselect(
            "选择条目", opts_ref,
            default=[opts_ref[i] for i in st.session_state.db_selected_idx["ref_file"]] if opts_ref else [],
            key="sel_ref_file",
            disabled=len(opts_ref) == 0
        )
        st.session_state.db_selected_idx["ref_file"] = [opts_ref.index(s) for s in sel_ref] if opts_ref else []
        if not opts_ref:
            st.info("暂无数据")
...
# 6 5 设计条件及要求
    ...
    selected_eq = st.multiselect(
        "选择【设备分级】条目", opts_eq,
        default=[opts_eq[i] for i in st.session_state.db_selected_idx["equip_class"]] if opts_eq else [],
        key="sel_equip_class",
        disabled=len(opts_eq) == 0
    )
    st.session_state.db_selected_idx["equip_class"] = [opts_eq.index(s) for s in selected_eq] if opts_eq else []
    if not opts_eq:
        st.info("未检测到【设备分级】数据库文件")
...
# 14 13 附录【固定DOM版，零节点增删】
    ...
    st.session_state.form["appendix_a"] = st.text_area(
        ...
        disabled=not opts["is_trans_design"]
    )
    st.divider()
    ...
    sel_if = st.multiselect(
        "选择接口清单条目",
        opts_if,
        default=[opts_if[i] for i in st.session_state.db_selected_idx["interface"]] if opts_if else [],
        key="sel_interface",
        disabled=not opts["have_append_b"] or len(opts_if) == 0
    )
    st.session_state.db_selected_idx["interface"] = [opts_if.index(s) for s in sel_if] if opts_if else []
    if not opts_if:
    ...
    st.session_state.form["appendix_c"] = st.text_area(
        ...
        disabled=not opts["have_append_c"]
    )
    st.divider()

    # 固定3个自定义附录，DOM完全固定
    st.subheader("📎 自定义附录（D/E/F）")
    for i in range(3):
        item = st.session_state.dyn_appendix[i]
        letter = item["letter"]
